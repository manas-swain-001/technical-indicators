...
import { createMACDState, updateMACD, getMACDValues } from '../indicators/trend/macd.js';
...
console.log(`Loaded ${candles.close.length} candles. Calculating MACD step by step...\n`);

// Initialize state with default parameters
const state = createMACDState();
...
for (let i = 0; i < candles.close.length; i++) {
    ...
    // Update the state
    updateMACD(state, candle);
    
    // Get current value
    const currentVal = getMACDValues(state);
    ...
    console.log(`[Candle ${String(i + 1).padStart(3, '0')}] | Date: ${date} | Close: ${candle.close.toFixed(2)} | MACD: ${valStr}`);
}

console.log("\nDone processing all candles for MACD.");
