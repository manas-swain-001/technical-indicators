...
try {
    ...
    if (text.includes('const candles =')) {
        ...
        console.log(`Loaded ${candles.close ? candles.close.length : 'mock'} candles. Calculating Accelerator Oscillator (AC)...\n`);
        
        const state = createACState(5, 34, 5);
        ...
        for (let i = 0; i < (candles.close.length || 0); i++) {
            const candle = {
                ...
                low: candles.low ? candles.low[i] : candles.close[i]*0.99,
            ...
            updateAC(state, candle);
            const val = getACValues(state);
            ...
            if (val === null) {
                tlStr += ` | AC: Gathering bounds...`;
            } else {
                tlStr += ` | AC: ${val.ac.toFixed(4)} | Direction: ${val.direction}`;
            }
            console.log(tlStr);
        }
        console.log("\nDone processing all candles for AC.");
    }
} catch(e) {
    console.error(e);
}
