...
import { createCCIState, updateCCI, getCCIValue } from '../indicators/momentum/cci.js';
...
console.log(`Loaded ${candles.close.length} candles. Calculating CCI step by step...\n`);

// Initialize state with default parameters
const state = createCCIState();
...
for (let i = 0; i < candles.close.length; i++) {
    ...
    // Update the state
    updateCCI(state, candle);
    
    // Get current value
    const currentVal = getCCIValue(state);
    ...
    console.log(`[Candle ${String(i + 1).padStart(3, '0')}] | Date: ${date} | Close: ${candle.close.toFixed(2)} | CCI: ${valStr}`);
}

console.log("\nDone processing all candles for CCI.");
