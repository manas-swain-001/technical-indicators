...
import { createATRState, updateATR, getATRValue } from '../indicators/volatility/atr.js';
...
console.log(`Loaded ${candles.close.length} candles. Calculating ATR step by step...\n`);

// Initialize state with default parameters
const state = createATRState();
...
for (let i = 0; i < candles.close.length; i++) {
    ...
    // Get current value
    const currentVal = getATRValue(state);
    ...
    console.log(`[Candle ${String(i + 1).padStart(3, '0')}] | Date: ${date} | Close: ${candle.close.toFixed(2)} | ATR: ${valStr}`);
}

console.log("\nDone processing all candles for ATR.");
