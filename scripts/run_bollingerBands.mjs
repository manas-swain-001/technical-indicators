...
import { createBollingerBandsState, updateBollingerBands, getBollingerBandsValues } from '../indicators/volatility/bollingerBands.js';
...
console.log(`Loaded ${candles.close.length} candles. Calculating BOLLINGERBANDS step by step...\n`);

// Initialize state with default parameters
const state = createBollingerBandsState();
...
for (let i = 0; i < candles.close.length; i++) {
    ...
    // Update the state
    updateBollingerBands(state, candle);
    
    // Get current value
    const currentVal = getBollingerBandsValues(state);
    ...
    console.log(`[Candle ${String(i + 1).padStart(3, '0')}] | Date: ${date} | Close: ${candle.close.toFixed(2)} | BOLLINGERBANDS: ${valStr}`);
}

console.log("\nDone processing all candles for BOLLINGERBANDS.");
