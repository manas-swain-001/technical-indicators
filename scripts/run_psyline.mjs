...
try {
    ...
    if (text.includes('const candles =')) {
        ...
        console.log(`Loaded ${candles.close ? candles.close.length : 'mock'} candles. Calculating Psychological Line (12)...\n`);
        
        const state = createPsyLineState(12);
        ...
        for (let i = 0; i < (candles.close.length || 0); i++) {
            ...
            updatePsyLine(state, candle);
            const val = getPsyLineValues(state);
            ...
            if (val === null) {
                tlStr += ` | PsyLine: Gathering bounds...`;
            } else {
                tlStr += ` | PsyLine: ${val.psy.toFixed(2)}% | Direction: ${val.direction}`;
            }
            console.log(tlStr);
        }
        console.log("\nDone processing all candles for Psychological Line.");
    }
} catch(e) {
    console.error(e);
}
