...
import { createRSIState, updateRSI, getRSIValue } from '../indicators/trend/rsi.js';
...
console.log(`Loaded ${candles.close.length} candles. Calculating RSI step by step...\n`);
...
for (let i = 0; i < candles.close.length; i++) {
    ...
    // Update the state
    updateRSI(state, candle);
    
    // Get current value
    const currentVal = getRSIValue(state);
    ...
    console.log(`[Candle ${String(i + 1).padStart(3, '0')}] | Date: ${date} | Close: ${candle.close.toFixed(2)} | RSI: ${valStr}`);
}

console.log("\nDone processing all candles for RSI.");
