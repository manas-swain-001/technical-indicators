...
import { createWilliamsRState, updateWilliamsR, getWilliamsRValue } from '../indicators/momentum/williamsR.js';
...
console.log(`Loaded ${candles.close.length} candles. Calculating WILLIAMSR step by step...\n`);

// Initialize state with default parameters
const state = createWilliamsRState();
...
for (let i = 0; i < candles.close.length; i++) {
    ...
    // Update the state
    updateWilliamsR(state, candle);
    
    // Get current value
    const currentVal = getWilliamsRValue(state);
    ...
    console.log(`[Candle ${String(i + 1).padStart(3, '0')}] | Date: ${date} | Close: ${candle.close.toFixed(2)} | WILLIAMSR: ${valStr}`);
}

console.log("\nDone processing all candles for WILLIAMSR.");
