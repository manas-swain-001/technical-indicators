...
import { createADXState, updateADX, getADXValues } from '../indicators/momentum/adx.js';
...
console.log(`Loaded ${candles.close.length} candles. Calculating ADX step by step...\n`);

// Initialize state with default parameters
const state = createADXState();
...
for (let i = 0; i < candles.close.length; i++) {
    ...
    // Update the state
    updateADX(state, candle);
    
    // Get current value
    const currentVal = getADXValues(state);
    ...
    console.log(`[Candle ${String(i + 1).padStart(3, '0')}] | Date: ${date} | Close: ${candle.close.toFixed(2)} | ADX: ${valStr}`);
}

console.log("\nDone processing all candles for ADX.");
