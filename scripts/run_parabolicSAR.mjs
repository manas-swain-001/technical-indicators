...
import { createPSARState, updatePSAR, getPSARValue } from '../indicators/other/parabolicSAR.js';
...
console.log(`Loaded ${candles.close.length} candles. Calculating PARABOLICSAR step by step...\n`);

// Initialize state with default parameters
const state = createPSARState();
...
for (let i = 0; i < candles.close.length; i++) {
    ...
    // Update the state
    updatePSAR(state, candle);
    
    // Get current value
    const currentVal = getPSARValue(state);
    ...
    console.log(`[Candle ${String(i + 1).padStart(3, '0')}] | Date: ${date} | Close: ${candle.close.toFixed(2)} | PARABOLICSAR: ${valStr}`);
}

console.log("\nDone processing all candles for PARABOLICSAR.");
