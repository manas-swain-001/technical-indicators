...
console.log(`Loaded ${candles.close.length} candles. Calculating STOCHASTIC step by step...\n`);

// Initialize state with default parameters
const state = createStochasticState();
...
for (let i = 0; i < candles.close.length; i++) {
    ...
    // Update the state
    updateStochastic(state, candle);
    
    // Get current value
    const currentVal = getStochasticValues(state);
    ...
    console.log(`[Candle ${String(i + 1).padStart(3, '0')}] | Date: ${date} | Close: ${candle.close.toFixed(2)} | STOCHASTIC: ${valStr}`);
}

console.log("\nDone processing all candles for STOCHASTIC.");
