...
import { createCMFState, updateCMF, getCMFValue } from '../indicators/volume/cmf.js';
...
console.log(`Loaded ${candles.close.length} candles. Calculating CMF step by step...\n`);

// Initialize state with default parameters
const state = createCMFState();
...
for (let i = 0; i < candles.close.length; i++) {
    ...
    // Update the state
    updateCMF(state, candle);
    
    // Get current value
    const currentVal = getCMFValue(state);
    ...
    console.log(`[Candle ${String(i + 1).padStart(3, '0')}] | Date: ${date} | Close: ${candle.close.toFixed(2)} | CMF: ${valStr}`);
}

console.log("\nDone processing all candles for CMF.");
