...
import { createEMAState, updateEMA, getEMAValue } from '../indicators/trend/ema.js';
...
console.log(`Loaded ${candles.close.length} candles. Calculating EMA step by step...\n`);

// Initialize state with default parameters
const state = createEMAState();
...
for (let i = 0; i < candles.close.length; i++) {
    ...
    // Update the state
    updateEMA(state, candle);
    
    // Get current value
    const currentVal = getEMAValue(state);
    ...
    console.log(`[Candle ${String(i + 1).padStart(3, '0')}] | Date: ${date} | Close: ${candle.close.toFixed(2)} | EMA: ${valStr}`);
}

console.log("\nDone processing all candles for EMA.");
