...
try {
    ...
    if (text.includes('const candles =')) {
        ...
        console.log(`Loaded ${candles.close ? candles.close.length : 'mock'} candles. Calculating Awesome Oscillator (AO)...\n`);
        
        const state = createAOState(5, 34);
        ...
        for (let i = 0; i < (candles.close.length || 0); i++) {
            ...
            updateAO(state, candle);
            const val = getAOValues(state);
            ...
            if (val === null) {
                tlStr += ` | AO: Gathering bounds...`;
            } else {
                tlStr += ` | AO: ${val.ao.toFixed(4)} | Direction: ${val.direction}`;
            }
            console.log(tlStr);
        }
        console.log("\nDone processing all candles for AO.");
    }
} catch(e) {
    console.error(e);
}
