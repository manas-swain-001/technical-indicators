...
import { createVWAPState, updateVWAP, getVWAPValue } from '../indicators/volume/vwap.js';
...
console.log(`Loaded ${candles.close.length} candles. Calculating VWAP step by step...\n`);

// Initialize state with default parameters
const state = createVWAPState();
...
for (let i = 0; i < candles.close.length; i++) {
    ...
    // Update the state
    updateVWAP(state, candle);
    
    // Get current value
    const currentVal = getVWAPValue(state);
    ...
    console.log(`[Candle ${String(i + 1).padStart(3, '0')}] | Date: ${date} | Close: ${candle.close.toFixed(2)} | VWAP: ${valStr}`);
}

console.log("\nDone processing all candles for VWAP.");
