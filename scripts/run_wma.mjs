...
import { createWMAState, updateWMA, getWMAValue } from '../indicators/trend/wma.js';
...
console.log(`Loaded ${candles.close.length} candles. Calculating WMA step by step...\n`);

// Initialize state with default parameters
const state = createWMAState();
...
for (let i = 0; i < candles.close.length; i++) {
    ...
    // Update the state
    updateWMA(state, candle);
    
    // Get current value
    const currentVal = getWMAValue(state);
    ...
    console.log(`[Candle ${String(i + 1).padStart(3, '0')}] | Date: ${date} | Close: ${candle.close.toFixed(2)} | WMA: ${valStr}`);
}

console.log("\nDone processing all candles for WMA.");
